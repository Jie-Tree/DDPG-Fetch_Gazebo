...
class ANet(nn.Module):  # ae(s)=a
    def __init__(self):
        ...
        self.fc1 = nn.Linear(1000 + 8, 520)  # action joints 8
...
class CNet(nn.Module):  # ae(s)=a
    def __init__(self):
        ...
        self.resnet50 = models.resnet50(True)  # (1, 1000)
        self.fc1 = nn.Linear(1000 + 8 + 8, 520)
        self.fc2 = nn.Linear(520, 64)
        self.fc3 = nn.Linear(64, 8)
...
class DDPG(object):
    def __init__(self):
        ...
        try:
            ...
            for fi in os.listdir('memory/'):
                ...
                fl.append(int(c))
            print fl
            self.memory_counter = max(fl)  # 记忆库计数
            print self.memory_counter
            self.memory = np.load('memory/' + str(self.memory_counter) + '.npy')
            print "load memory {}.npy".format(self.memory_counter)
        except:
            print "no memory"
            self.memory = np.zeros((1000, (224 * 224 * 3 + 8) * 2 + 8 + 1))  # s, a, r, s_
    ...
    def choose_action(self, s):
        ...
        joint_view = torch.from_numpy(np.array(joint_view).reshape(-1, 8))
        action = self.Actor_eval.forward(image_view_rgb, joint_view).detach()
        action = action.cpu().numpy()
        aa = np.clip(action, -0.1, 0.1)
        return [aa[0][0], aa[0][1], aa[0][2], aa[0][3], aa[0][4], aa[0][5], aa[0][6], aa[0][7]]

    def learn(self):
        sample_index = np.random.choice(MEMORY_CAPACITY, BATCH_SIZE)
        b_memory = self.memory[sample_index, :]
        b_joint1 = torch.FloatTensor((b_memory[:, :8]).reshape(-1, 8))
        rgb_np = (b_memory[:, 8:N_STATES + 8]).reshape(-1, 224, 224, 3)[:, :, :, :3]
        b_rgb2 = torch.FloatTensor(rgb_np).permute(0, 3, 1, 2)
        # b_s = b_s1, b_s2
        # observation 2
        print '---step1--'
        b_a = torch.LongTensor((b_memory[:, N_STATES + 8:N_STATES + 16]).reshape(-1, 8).astype(float))
        b_r = torch.FloatTensor((b_memory[:, N_STATES + 16:N_STATES + 17]).reshape(-1, 1))
        b_joint_1 = torch.FloatTensor((b_memory[:, N_STATES + 17:N_STATES + 25]).reshape(-1, 8))
    ...
    def store_transition(self, s, a, r, s_):
        ...
        s3, s4 = s_
        s1 = np.array(s1).reshape(-1, 8)
        s2 = np.array(s2).reshape(-1, 224 * 224 * 3)
        r = np.array(r).reshape(-1, 1)
        s3 = np.array(s3).reshape(-1, 8)
    ...
    def save_model(self):
        # c = 0
        # for fi in os.listdir('models/Actor_eval_ddpg/'):
        #     c = fi.split('.')[0]
        
        # print("model_number:", model_number)
        torch.save(self.Actor_eval, "models/Actor_eval_ddpg/" + str(self.model_number) + ".pkl")
        torch.save(self.Actor_target, "models/Actor_target_ddpg/" + str(self.model_number) + ".pkl")
        torch.save(self.Critic_eval, "models/Critic_eval_ddpg/" + str(self.model_number) + ".pkl")
        torch.save(self.Critic_target, "models/Critic_target_ddpg/" + str(self.model_number) + ".pkl")
        self.model_number += 1
    ...
    def load_model(self):
        ...
        # print("model_number:", model_number)
        # try:
        torch.load(self.Actor_eval, "models/Actor_eval_ddpg/" + str(self.model_number) + ".pkl")
        torch.load(self.Actor_target, "models/Actor_target_ddpg/" + str(self.model_number) + ".pkl")
        torch.load(self.Critic_eval, "models/Critic_eval_ddpg/" + str(self.model_number) + ".pkl")
        torch.load(self.Critic_target, "models/Critic_target_ddpg/" + str(self.model_number) + ".pkl")
